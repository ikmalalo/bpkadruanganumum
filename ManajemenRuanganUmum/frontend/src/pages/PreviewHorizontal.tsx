            ...
            {currentSlide?.type === 'AGENDA' ? (
              ...
              <thead className="animate-slide-right">
                <tr className="bg-orange-500 text-white flex w-full">
                  <th className="py-2 px-2 text-center text-sm font-black w-14 md:w-16 flex items-center justify-center">NO</th>
                  <th className="py-2 px-4 text-left text-sm font-black flex-1 flex items-center">HARI / TANGGAL</th>
                  <th className="py-2 px-4 text-left text-sm font-black flex-[1.2] flex items-center">TEMPAT</th>
                  <th className="py-2 px-4 text-center text-sm font-black w-24 md:w-32 flex items-center justify-center">PUKUL</th>
                  ...
                  <th className="py-2 px-4 text-center text-sm font-black w-24 md:w-32 flex items-center justify-center">STATUS</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 flex-1 flex flex-col w-full overflow-hidden">
                {currentSlide.data.map((item, idx) => (
                  ...
                  >
                    <td className="px-2 text-base font-black text-gray-400 text-center w-14 md:w-16 flex items-center justify-center">{idx + 1}</td>
                    ...
                    <td className="px-4 flex-[1.2] flex items-center min-w-0 font-black text-[13px] text-gray-800 uppercase whitespace-nowrap overflow-hidden text-ellipsis">{item.tempat}</td>
                    <td className="px-4 w-24 md:w-32 flex items-center justify-center"><span className="text-sm md:text-base font-black text-orange-500 bg-orange-50 px-2 py-1 md:px-2.5 rounded-md">{item.pukul}</span></td>
                    ...
                    <td className="px-4 flex-1 flex items-center min-w-0 font-black text-[12px] md:text-[13px] text-gray-800 uppercase">{item.pelaksana}</td>
                    <td className="px-4 w-24 md:w-32 flex items-center justify-center">
                      <span className={`inline-block w-full py-2 rounded-xl text-[12px] md:text-sm font-black text-white text-center ${item.status === 'Berlangsung' ? 'bg-[#10b981]' : 'bg-[#3b82f6]'}`}>{item.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
