            ...
            {currentSlide?.type === 'AGENDA' ? (
              ...
              <thead className="animate-slide-right">
                <tr className="bg-orange-500 text-white flex w-full">
                  ...
                  <th className="py-2 px-4 text-left text-[10px] md:text-sm font-black w-[15%] min-w-[100px] flex items-center">HARI / TANGGAL</th>
                  <th className="py-2 px-4 text-left text-[10px] md:text-sm font-black flex-1 min-w-[120px] flex items-center">TEMPAT</th>
                  <th className="py-2 px-4 text-center text-[10px] md:text-sm font-black w-[15%] min-w-[90px] flex items-center justify-center">PUKUL</th>
                  <th className="py-2 px-6 text-left text-[10px] md:text-sm font-black flex-[2] min-w-[200px] flex items-center">ACARA</th>
                  <th className="py-2 px-4 text-left text-[10px] md:text-sm font-black w-[15%] min-w-[120px] hidden lg:flex items-center">PELAKSANA</th>
                ...
                {currentSlide.data.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 w-[15%] min-w-[100px] flex flex-col justify-center min-w-0">
                      <span className="text-[10px] md:text-sm font-black text-gray-800 uppercase leading-none mb-1">{item.hari}</span>
                      <span className="text-[9px] md:text-[11px] font-bold text-gray-400 uppercase tracking-wider">{item.tanggal.split(', ')[1] || item.tanggal}</span>
                    </td>
                    <td className="px-4 flex-1 min-w-[120px] flex items-center font-black text-[10px] md:text-xs lg:text-sm text-gray-800 uppercase leading-tight">{item.tempat}</td>
                    <td className="px-4 w-[15%] min-w-[90px] flex items-center justify-center"><span className="text-[10px] md:text-sm lg:text-base font-black text-orange-500 bg-orange-50 px-1.5 md:px-2.5 py-1 rounded-md whitespace-nowrap">{item.pukul}</span></td>
                    ...
                    <td className="px-2 md:px-4 w-[12%] min-w-[90px] flex items-center justify-center">
                      <span className={`inline-block w-full py-1 md:py-2 rounded-lg md:rounded-xl text-[9px] md:text-sm font-black text-white text-center ${item.status === 'Berlangsung' ? 'bg-[#10b981]' : 'bg-[#3b82f6]'}`}>{item.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
